...
[ $arquitetura = "aarch64" ] && arquivo_compactado=$(find / -type f -iregex \
                              ".*aarch64-qbittorrent-nox.7z" 2> /dev/null   \
                              |head -n1 | xargs realpath)

[ $arquitetura = "x86_64" ] && arquivo_compactado=$(find / -type f -iregex  \
                              ".*x86_64-qbittorrent-nox.7z" 2> /dev/null    \
                              |head -n1 | xargs realpath)


[ $arquitetura = "aarch64" ] && arquivo="aarch64-qbittorrent-nox"
[ $arquitetura = "x86_64" ]  && arquivo="x86_64-qbittorrent-nox"
